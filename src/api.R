...
# Deploy with: Rscript -e "plumber::plumb('src/api.R')$run(port=8000, host='0.0.0.0')"
# ============================================================================
...
tryCatch({
  ...
  if (!file.exists(MODEL_PATH)) {
    stop("Model file not found. Run src/TRAIN_IBM_TELCO_85.R first to generate '", MODEL_PATH, "'")
  }
  model_artifact <- readRDS(MODEL_PATH)
  cat("Model loaded successfully. AUC:", model_artifact$auc, "\n")
}, error = function(e) {
  cat("Error during startup:", e$message, "\n")
  quit(status = 1)
})
...
function() {
  tryCatch({
    ...
    list(error = "Health check failed", details = e$message)
  })
}
...
function(req) {
  tryCatch({
    ...
    list(error = "Batch prediction failed", details = e$message)
  })
}
